 ...
 <div class="md:w-1/2 bg-gray-50 p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col sm:flex-row items-center gap-6">
  ...
  <div class="flex-1 space-y-2 w-full">
    <div v-for="i in 5" :key="i" class="flex items-center gap-2">
      ...
      <!-- Reviews List with horizontal scroll if > 2 rows -->
...
const avgRating = computed(()=>product.value.reviews.reduce((a,b)=>a+b.rating,0)/product.value.reviews.length)
const getPercentage = (star)=> (product.value.reviews.filter(r=>r.rating===star).length / product.value.reviews.length)*100

// Split reviews into chunks of 2 (2 rows per column)
const reviewChunks = computed(()=>{
  const chunks = []
  for(let i=0;i<product.value.reviews.length;i+=2) chunks.push(product.value.reviews.slice(i,i+2))
  return chunks
})
</script>
