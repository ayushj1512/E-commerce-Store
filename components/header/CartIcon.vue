<template>
  ...
  >
    ...
    <!-- ✅ FIXED Transition: ensure only one root element -->
    <transition name="fade-scale">
      <div v-if="showMiniCart" class="mini-cart-wrapper">
        <div class="absolute right-0 mt-2 w-80 bg-white shadow-lg border rounded-xl p-4 z-50">
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  <p v-if="item.size" class="text-xs text-gray-500">
                    Size: {{ item.size }}
                  </p>
...
const navigateToProduct = (item) => {
  const parentSlug = item.parentCategory
    ? item.parentCategory.toLowerCase()
    : 'collection'
  const subSlug = item.subCategory
...
/* wrapper ensures only 1 root node under transition */
.mini-cart-wrapper {
  position: relative;
}
</style>
